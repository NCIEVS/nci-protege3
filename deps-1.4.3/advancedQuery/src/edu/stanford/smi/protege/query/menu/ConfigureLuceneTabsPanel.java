...
public class ConfigureLuceneTabsPanel extends AbstractValidatableComponent {
    ...
    private JComboBox querySearchType;
    ...
    private void addQuerySearchType() {
    	querySearchType = new JComboBox();
    	for (String s : QueryComponent.DEFAULT_TYPES) {
    		querySearchType.addItem(s);
    	}
    	
    	querySearchType.setSelectedItem(configuration.getDefaultQuerySearchType());
    	querySearchType.addActionListener(new ActionListener() {
			
			public void actionPerformed(ActionEvent e) {
				String name = (String) querySearchType.getSelectedObjects()[0];
		    	for (String s : QueryComponent.DEFAULT_TYPES) {
		    		if (s.equals(name)) {
						configuration.setDefaultQuerySearchType(s);
		    		}
		    	}
			}
		});
        LabeledComponent lc = new LabeledComponent("Default Search Type for Query", querySearchType);
        lc.setAlignmentX(Component.LEFT_ALIGNMENT);
        centerComponent.add(lc);
    }
    ...
    public String getDefaultQuerySearchType() {
    	return configuration.getDefaultQuerySearchType();
    	
    	/**
    	String name = (String) querySearchType.getSelectedObjects()[0];
    	for (SearchType st : SearchType.values()) {
    		if (st.getName().equals(name)) {
				configuration.setDefaultQuerySearchType(st);
    		}
    	}
    	**/
